...
async def medsbuddy_agent_router(req: AgentRouterRequest) -> dict[str, Any]:
    ...
    if req.mode == "doctor_visit_live":
        system_prompt = (
            ...
            "set shouldRespond true and response exactly: 'No further questions, Doctor. The care plan is complete and today’s visit summary is ready.' "
        ...
        system_prompt = (
            ...
            "save health data, prepare doctor visit context, generate QR flow only when explicitly requested, "
            "create chats, and answer normally. "
            ...
            "timeline, onset, duration, patientNotes. For doctor_visit_prep, set navigateTo '/doctor', "
            "needsSave true, and write a brief confirmation only. Do not offer QR generation, do not ask a "
            "follow-up question, and do not end with 'Would you like'. Use generate_qr only when the user "
            "explicitly asks for QR, emergency QR, or QR card. "

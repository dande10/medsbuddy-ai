...
async def medsbuddy_agent_router(req: AgentRouterRequest) -> dict[str, Any]:
    ...
    if req.mode == "doctor_visit_live":
        system_prompt = (
            ...
            "Set includePreviousVisitHistory true only when previous visit or medical history is requested. "
            "If the doctor asks whether the patient has any questions before ending the visit, do not repeat "
            "the approved pre-visit summary. Respond only if a clinically important care-plan field is missing; "
            "otherwise set shouldRespond false. "
